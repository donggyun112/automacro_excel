...
class SettingsDialog(QDialog):
    ...
    def choose_save_path(self):
        ...
        while True:
            save_path, _ = QFileDialog.getSaveFileName(self, "결과 파일 저장", "", "Excel Files (*.xlsx *.xls)")
            if not save_path:
                return
            if not save_path.endswith(".xlsx") and not save_path.endswith(".xls"):
                QMessageBox.critical(self, "오류", "올바른 파일 형식을 선택해주세요.")
                continue
...
class ExcelHelper(QMainWindow):
    ...
    def browse_file(self):
        self.path, _ = QFileDialog.getOpenFileName(self, "Excel 파일 선택", "", "Excel Files (*.xlsx *.xls)")
        if self.path:
            try:
