...
class CompareMerge(QDialog):
    ...
    def update_button_states(self):
        ...
        # 선택한 파일의 시트 이름 가져오기
        selected_file_sheet_names = self.workbook.sheetnames if file_selected else []
    ...
    def browse_workbook(self):
        path, _ = QFileDialog.getOpenFileName(self, "Excel 파일 선택", "", "Excel Files (*.xlsx)")
        if path:
            if self.workbook:
                self.workbook.close()
                self.workbook = None
    ...
    def compare_dataframes(self, df1, df2):
        try:
            df1 = np.array(list(df1))
            df2 = np.array(list(df2))
            # 데이터프레임이 비어있는지 확인
            ...
            # 열 개수 비교
            col_similarity = min(df1.shape[1], df2.shape[1]) / max(df1.shape[1], df2.shape[1])

            # 행 개수 비교
            row_similarity = min(df1.shape[0], df2.shape[0]) / max(df1.shape[0], df2.shape[0])

            # 공통 열만 선택
            common_cols = [col for col in range(df1.shape[1]) if col < df2.shape[1]]
            df1 = df1[:, common_cols]
            df2 = df2[:, common_cols]

            # 행 개수 맞추기
            max_rows = max(df1.shape[0], df2.shape[0])
            df1 = np.pad(df1, ((0, max_rows - df1.shape[0]), (0, 0)), mode='constant', constant_values=np.nan)
            df2 = np.pad(df2, ((0, max_rows - df2.shape[0]), (0, 0)), mode='constant', constant_values=np.nan)

            # 값 유사도 계산 (숫자형 열 제외)
            value_similarities = []
            non_numeric_weight = 0.8  # 숫자형이 아닌 데이터의 가중치
            for col in range(df1.shape[1]):
                if df1[:, col].dtype == object:
                    similarity = np.mean(df1[:, col].astype(str) == df2[:, col].astype(str))
                    value_similarities.append(similarity * non_numeric_weight)
                else:
                    similarity = 1 - np.mean(np.abs(df1[:, col] - df2[:, col]))
                    value_similarities.append(similarity)
            value_similarity = np.mean(value_similarities) if value_similarities else 0

            # 전체 유사도 계산 (가중치 조정 가능)
            total_similarity = (col_similarity + row_similarity + value_similarity) / 3

            return total_similarity

        except Exception as e:
            print(f"데이터프레임 유사도 계산 중 오류 발생: {str(e)}")
            return 0.0
    ...
    def execute_compare_merge(self):
        ...
        if not selected_sheets:
            ...
            return
        try:
            wb1_path = self.parent.workbook.fullname
            wb2_path = self.path
            if self.parent.save_path and  self.parent.save_path.endswith(".xlsx") and self.parent.save_path != "":
                output_path = self.parent.save_path
            else:
                while True:
                    output_path, _ = QFileDialog.getSaveFileName(self, "결과 파일 저장", "", "Excel Files (*.xlsx)")
                    if not output_path:
                        return 
                    ...
                    if output_path:
                        ...
                        return
            wb1 = load_workbook(wb1_path, data_only=False)
            wb2 = load_workbook(wb2_path, data_only=False)
        except Exception as e:
        ...
        error_messages = []  # 오류 메시지를 저장할 리스트
        ...
        for sheet_name in self.selected_sheets:
            ...
            if range_value == "All":
                ...
                if self.parent.workbook:
                    ...
                    if compare_sheet:
                        compare_data = wb2[compare_sheet].values
                        ...
                        if status:
                            ...
                            error_messages.append(f"시트 '{sheet_name}'에서 병합 오류 발생")
                    else:
                ...
                if self.parent.workbook:
                    ...
                    if compare_sheet:
                        ...
                        for range_item in range_list:
                            ...
                            print(f"선택한 범위: {sheet_range.address}")
                            compare_range = CellRange(range_item.strip())
                            compare_data = [[cell.value for cell in row] for row in wb2[compare_sheet][compare_range.min_row:compare_range.max_row+1][compare_range.min_col:compare_range.max_col+1]]
                            print(compare_data)
                            if not self.compare_and_merge(data, compare_data, sheet_range, sheet_name, wb1, wb2, None, error_messages):
                                merge_success = False
    ...
    def find_compare_sheet(self, sheet_name):
        ...
        if compare_workbook:
            for sheet_name_with_number in compare_workbook.sheetnames:
                compare_sheet_name_without_number = re.sub(r'^\d+\.\s*', '', sheet_name_with_number)
    ...
    def compare_and_merge(self, data, compare_data, sheet_range, sheet_name, wb1, wb2, _range_, error_messages):
        ...
        # 병합 수행
        if _range_ != "All":
            range__ = CellRange(sheet_range.address)
        else:
            range__ = None
